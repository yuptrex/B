...
# reaction list — arbitrary Unicode emoji are rejected by the API with a
# BadRequest (REACTION_INVALID). This is a large, verified-valid subset of
# that list (not the complete set) so reactions stay varied without risking
# silent failures from an unrecognized emoji.
REACTION_EMOJIS = [
    "👍", "👎", "❤️", "🔥", "🥰", "👏", "😁", "🤔",
    "🤯", "😱", "🎉", "🤩", "🙏", "👌", "🕊️", "🥱",
    "🥴", "😍", "🐳", "🌚", "🌭", "💯", "🤣", "⚡️",
...
async def react_to_message(message) -> None:
    ...
    never block or break the bot's actual reply.
    """
    try:
        emoji = random.choice(REACTION_EMOJIS)
        await message.set_reaction(reaction=emoji, is_big=True)
    except Exception:
        logger.exception("Failed to set reaction on message %s", message.message_id)
